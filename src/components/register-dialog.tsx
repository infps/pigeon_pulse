...
function resetForm() {
  return {
    ...
    selectedLoft: "",
    reservedBirds: 0,
    birdSlots: [] as BirdSlot[],
    payments: [] as PaymentData[],
    breederQuery: "",
    lookedUpBreeder: null as null | {
      id: number;
...
export function RegisterDialog({ event, eventId, open, onOpenChange, onSuccess }: RegisterDialogProps) {
  ...
  const handleBreederCreated = (newBreeder: any) => {
    queryClient.invalidateQueries({ queryKey: ["breeders"] });
    // Auto-select newly created breeder
    if (newBreeder?.breeder?.id) {
      setLookedUpBreeder(newBreeder.breeder);
      handleBreederChange(String(newBreeder.breeder.id));
    }
  };
  ...
  const addExistingBird = (bird: any) => {
    if (birdSlots.length >= reservedBirds) {
      toast.error(`Maximum ${reservedBirds} birds`);
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedBreederId) { toast.error("Select a breeder"); return; }
    if (!selectedLoft) { toast.error("Select a loft/team"); return; }
    if (reservedBirds === 0) { toast.error("Enter number of reserved birds"); return; }
    ...
    for (let i = 0; i < birdSlots.length; i++) {
      ...
      if (slot.type === "new") {
        if (!slot.name || !slot.color || !slot.band1 || !slot.band2 || !slot.band3 || !slot.band4) {
          toast.error(`Fill all fields for new bird ${i + 1}`);
          return;
        }
      }
    }
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl !flex flex-col max-h-[90vh] overflow-hidden">
          <DialogHeader className="shrink-0">
            <DialogTitle>Register Breeder</DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
            <div className="flex-1 overflow-y-auto space-y-6 pr-1">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold text-sm">Breeder</h3>
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
                ...
                {lookedUpBreeder ? (
                  ...
                  <div className="flex gap-2">
                    <Input
                      placeholder="Username or breeder ID"
                      value={breederQuery}
                      onChange={(e) => setBreederQuery(e.target.value)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") { e.preventDefault(); lookupBreeder(); }
                      }}
                    />
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Select value={selectedLoft} onValueChange={setSelectedLoft}>
                      ...
                      <SelectContent>
                        {teams.length === 0 ? (
                          <div className="px-2 py-4 text-sm text-center text-muted-foreground">No teams</div>
                        ) : (
                          teams.map((team: any) => (
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (maxBirds > 0 && value > maxBirds) {
                        toast.error(`Max birds: ${maxBirds}`);
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="font-semibold text-sm">
                      ...
                      <span className="text-muted-foreground font-normal">
                        ({birdSlots.length}/{reservedBirds} — optional)
                      </span>
                    </h3>
                    <div className="flex gap-2">
                    ...
                    <div key={index} className="border rounded-lg p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <h4 className="font-medium text-sm">
                          Bird {index + 1} —{" "}
                          <span className="text-muted-foreground">{slot.type === "existing" ? "existing" : "new"}</span>
                        </h4>
                      ...
                      {slot.type === "existing" ? (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                          <div><p className="text-muted-foreground">Name</p><p className="font-medium">{slot.name || "—"}</p></div>
                          <div><p className="text-muted-foreground">Color</p><p className="font-medium">{slot.color || "—"}</p></div>
                          <div><p className="text-muted-foreground">Sex</p><p className="font-medium">{slot.sex}</p></div>
                          <div><p className="text-muted-foreground">Band</p><p className="font-medium">{slot.band}</p></div>
                        </div>
                      ) : (
                        <>
                          <div className="space-y-2">
                          ...
                          <div className="flex items-end gap-2">
                            <div className="flex-1 space-y-2">
                              ...
                              <Select value={slot.band1} onValueChange={(v) => updateNewBirdField(index, "band1", v)}>
                                <SelectTrigger className="h-9"><SelectValue placeholder="Fed" /></SelectTrigger>
                                <SelectContent>
                            ...
                            <div className="flex-1 space-y-2">
                              <Label>Year *</Label>
                              <Input value={slot.band2} onChange={(e) => updateNewBirdField(index, "band2", e.target.value.replace(/[^0-9]/g, ""))} className="h-9" required />
                            </div>
                            <span className="text-muted-foreground pb-2">-</span>
                            <div className="flex-1 space-y-2">
                              <Label>Letters *</Label>
                              <Input value={slot.band3} onChange={(e) => updateNewBirdField(index, "band3", e.target.value.toUpperCase())} className="h-9" required />
                            </div>
                            <span className="text-muted-foreground pb-2">-</span>
                            <div className="flex-1 space-y-2">
                              <Label>Band No. *</Label>
                              <Input value={slot.band4} onChange={(e) => updateNewBirdField(index, "band4", e.target.value)} className="h-9" required />
                            </div>
                            <span className="text-muted-foreground pb-2">-</span>
                            <div className="flex-1 space-y-2">
                              <Label>Color *</Label>
                              <Select value={slot.color} onValueChange={(v) => updateNewBirdField(index, "color", v)}>
                            ...
                            <div className="flex-1 space-y-2">
                              <Label>Sex *</Label>
                              <Select value={slot.sex} onValueChange={(v: "COCK" | "HEN" | "UNKNOWN") => updateNewBirdField(index, "sex", v)}>
                  ...
                  {birdSlots.length === 0 && (
                    <p className="text-sm text-muted-foreground border rounded-lg p-4 text-center">
                      No birds added. Add existing or new birds above, or register without birds.
                    </p>
                  )}
                </div>
              )}
                  ...
                  <div className="border rounded-lg p-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input type="number" step="0.01" value={payments[0].amountToPay} disabled className="bg-gray-100" />
                      </div>
                      <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(v: "CREDIT_CARD" | "PAYPAL" | "BANK_TRANSFER" | "CASH") => handlePaymentChange("method", v)}
                        >
            ...
            <div className="shrink-0 border-t pt-4 mt-2 flex justify-end gap-2">
              <Button type="button" variant="outline" onClick={() => onOpenChange(false)}>
                Cancel
              </Button>
              <Button type="submit" disabled={submitting || reservedBirds === 0}>
