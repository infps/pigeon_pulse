...
export function MapSearchControl({ onPick, onAddress }: Props) {
  ...
  const pick = (r: Result) => {
    // Suppress the ghost map click that fires when the dropdown unmounts under the cursor.
    const m = map as L.Map & { _ppSuppressClick?: boolean };
    m._ppSuppressClick = true;
    window.setTimeout(() => { m._ppSuppressClick = false; }, 400);
  ...
  return (
    <div ref={ref} className="absolute top-2 left-2 z-[1000] w-64">
      ...
      {results.length > 0 && (
        <ul className="absolute top-full mt-1 left-0 w-full bg-white rounded shadow max-h-48 overflow-auto text-sm">
          {results.map((r, i) => (
            <li key={i}>
              <button
                type="button"
